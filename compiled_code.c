...
double calculateROI(const double *returns, int numReturns);
double calculateAnnualizedReturn(const double *returns,int numReturns);
...
double calculateBeta(const double *portfolioReturns, const double *benchmarkReturns, int numReturns,int numYears);
...
int main(){
    ...
    scanf("%d",&time_months);
    numYears=ceil(time_months/12.0);
    ...
    printf("your intial investment is %.2lf",initialInvestment);//new
    ...
    double roi = calculateROI(transactionReturns,numReturns);
    printf("ROI: %.2f%%\n", roi * 100);

    double annualizedReturn = calculateAnnualizedReturn(transactionReturns,numReturns);
    ...
    double beta = calculateBeta(transactionReturns, benchmarkReturns, numReturns,numYears);
    printf("Beta: %.2f\n", beta);
    ...
    double cagr = calculateCAGR(transactionReturns,numReturns);
    printf("CAGR: %.2f%%\n", cagr * 100);
    ...
    if(downsideRisk!=0)
        ...
        printf("Your Portfolio has never taken any risk so sortino ratio cannot be calculated.\n");
...
float tot_tax(float amt){
    ...
    if((ans=='y')||(ans=='Y')){
        float temp;
        printf("Enter total amount of capital gains you made: ");
        scanf("%f",&temp);
        amt-=temp;
        tax+=ltcg(temp);
    }
...
float deductions(float amt){
    ...
    scanf("\n%c",&ans);
    if((ans=='Y')||(ans=='y')){
        printf("Enter amount of donations made that are 100%% deductible: ");
        scanf("%f",&temp);
        ded+=temp;
        printf("Enter amount of donations made that are 50%% deductible: ");
        scanf("%f",&temp);
        ded+=0.5*temp;
    }
    else if((ans=='n')||(ans=='N')){
...
double calculateROI(const double *returns, int numReturns) {
    double tot_returns=1;
    for(int i=0;i<numReturns;i++){
        tot_returns*=(1+returns[i]);
    }
    return tot_returns-1.0;

}

double calculateAnnualizedReturn(const double *returns,int numReturns) {
    double roi = calculateROI(returns, numReturns);
    return pow(1.0 + roi, 1.0 / numReturns) - 1.0;
}
...
double calculateCAGR(const double *returns, int numReturns) {
    double tot_returns=1;
    for(int i=0;i<numReturns;i++){
        tot_returns*=(1+returns[i]);
    }
    return pow(tot_returns,1.0/numReturns)-1.0;
}
